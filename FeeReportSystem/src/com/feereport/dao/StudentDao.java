...
public class StudentDao {
	public static int nextId() {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT max(id) as max_id FROM fee_student");
	...
	public static int save(StudentBean bean){
		...
		try{
			Connection con=DB.getCon();
			PreparedStatement ps=con.prepareStatement("insert into fee_student(id,roll_no,name,email,sex,course,fee,paid,due,address,contact) values(?,?,?,?,?,?,?,?,?,?,?)");
			ps.setInt(1, nextId);
			ps.setInt(2, bean.getRollno());
			ps.setString(3,bean.getName());
			ps.setString(4,bean.getEmail());
			ps.setString(5,bean.getSex());
			ps.setString(6,bean.getCourse());
			ps.setInt(7,bean.getFee());
			ps.setInt(8,bean.getPaid());
			ps.setInt(9,bean.getDue());
			ps.setString(10,bean.getAddress());
			ps.setString(11,bean.getContact());
